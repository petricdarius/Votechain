...
function AdminElections() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success("Alegerea a fost eliminată.");
    },
    onError: (err) => {
      toast.error(
        err.response?.data?.message || "Nu s-a putut șterge alegerea.",
      );
    },
  });
  return (
    <div>
      <div className="lg:col-span-2 bg-[#121620]/60 border border-white/5 backdrop-blur-xl p-6 rounded-2xl shadow-xl">
        <h2 className="text-lg font-bold uppercase tracking-wider text-gray-200 mb-6 flex items-center gap-2">
          <FileText className="text-blue-400 w-5 h-5" /> Evenimente Active în
          Sistem
        </h2>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12 text-gray-500 font-mono text-sm border border-dashed border-white/10 rounded-xl">
            Nu există nicio rundă de alegeri creată în baza de date.
          </div>
        ) : (
              ...
              >
                <div className="space-y-1">
                  ...
                  <div className="flex items-center gap-4 text-[10px] text-gray-500 font-mono pt-2">
                    ...
                    <span>•</span>
                    <span>
                      Candidati înscriși: {el.candidates?.length || 0}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 self-end md:self-center">
                  ...
                  <button
                    onClick={() => {
                      if (
                        confirm(`Sigur vrei să ștergi definitiv "${el.title}"?`)
                      ) {
